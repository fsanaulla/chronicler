...
/**
  ...
  * Date: 10.08.17
  */
class UserManagementSpec extends TestSpec with DockerizedInfluxDB with TryValues {
  ...
  "User management operation" should "create user" in {
    ...
    influx.showUsers.success.value.result.contains(UserInfo(userName, isAdmin = false)) shouldEqual true
  }

  it should "create admin" in {
    influx.createAdmin(admin, adminPass).success.value shouldEqual OkResult
    influx.showUsers.success.value.result.contains(UserInfo(admin, isAdmin = true)) shouldEqual true
  }

  it should "show user privileges" in {
    influx.showUserPrivileges(admin).success.value.result shouldEqual Nil
  }
  ...
  it should "set privileges" in {
    ...
    influx.setPrivileges("unknown", userDB, Privileges.READ).success.value.ex.value.getMessage shouldEqual "user not found"

    influx.showUserPrivileges(userName).success.value.result shouldEqual Array(UserPrivilegesInfo(userDB, Privileges.READ))
  }

  it should "revoke privileges" in {
    influx.revokePrivileges(userName, userDB, Privileges.READ).success.value shouldEqual OkResult
    influx.showUserPrivileges(userName).success.value.result shouldEqual Array(UserPrivilegesInfo(userDB, Privileges.NO_PRIVILEGES))
  }

  it should "disable admin" in {
    influx.disableAdmin(admin).success.value shouldEqual OkResult
    influx.showUsers.success.value.result.contains(UserInfo(admin, isAdmin = false)) shouldEqual true
  }
