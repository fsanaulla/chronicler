...
class UserManagementSpec extends TestSpec with DockerizedInfluxDB with FutureHandler {
  ...
  "User management operation" should "create user" in {
    ...
    influx.showUsers.futureValue.result.contains(UserInfo(userName, isAdmin = false)) shouldEqual true
  }

  it should "create admin" in {
    influx.createAdmin(admin, adminPass).futureValue shouldEqual OkResult
    influx.showUsers.futureValue.result.contains(UserInfo(admin, isAdmin = true)) shouldEqual true
  }

  it should "show user privileges" in {
    influx.showUserPrivileges(admin).futureValue.result shouldEqual Nil
  }
  ...
  it should "set privileges" in {
    ...
    influx.showUserPrivileges(userName).futureValue.result shouldEqual Array(UserPrivilegesInfo(userDB, Privileges.READ))
  }

  it should "revoke privileges" in {
    influx.revokePrivileges(userName, userDB, Privileges.READ).futureValue shouldEqual OkResult
    influx.showUserPrivileges(userName).futureValue.result shouldEqual Array(UserPrivilegesInfo(userDB, Privileges.NO_PRIVILEGES))
  }

  it should "disable admin" in {
    influx.disableAdmin(admin).futureValue shouldEqual OkResult
    influx.showUsers.futureValue.result.contains(UserInfo(admin, isAdmin = false)) shouldEqual true
  }
