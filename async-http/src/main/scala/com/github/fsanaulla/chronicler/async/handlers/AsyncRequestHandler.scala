...
private[fsanaulla] trait AsyncRequestHandler
  ...
  override def readRequest(uri: Uri, entity: Option[String] = None): Future[Response[JValue]] =
    sttp.get(uri).response(asJson).send()

  override def writeRequest(uri: Uri, entity: String): Future[Response[JValue]] =
