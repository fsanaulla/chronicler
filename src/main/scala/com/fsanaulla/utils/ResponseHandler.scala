...
private[fsanaulla] object ResponseHandler {
  ...
  def toCqQueryResult(response: HttpResponse)(implicit ex: ExecutionContext, mat: ActorMaterializer, reader: InfluxReader[ContinuousQuery]): Future[QueryResult[ContinuousQueryInfo]] = {
    toComplexQueryResult[ContinuousQuery, ContinuousQueryInfo](response, (name: String, seq: Seq[ContinuousQuery]) => ContinuousQueryInfo(name, seq))
  }

  def toShardQueryResult(response: HttpResponse)(implicit ex: ExecutionContext, mat: ActorMaterializer, reader: InfluxReader[Shard]): Future[QueryResult[ShardInfo]] = {
    toComplexQueryResult[Shard, ShardInfo](response, (name: String, seq: Seq[Shard]) => ShardInfo(name, seq))
  }

  def toSubscriptionQueryResult(response: HttpResponse)(implicit ex: ExecutionContext, mat: ActorMaterializer, reader: InfluxReader[Shard]): Future[QueryResult[SubscriptionInfo]] = {
    toComplexQueryResult[Subscription, SubscriptionInfo](response, (name: String, seq: Seq[Subscription]) => SubscriptionInfo(name, seq))
  }

  def toShardGroupQueryResult(response: HttpResponse)(implicit ex: ExecutionContext, mat: ActorMaterializer, reader: InfluxReader[Shard]): Future[QueryResult[ShardGroupsInfo]] = {
    toComplexQueryResult[ShardGroup, ShardGroupsInfo](response, (name: String, seq: Seq[ShardGroup]) => ShardGroupsInfo(name, seq))
  }

  private def toComplexQueryResult[A, B](response: HttpResponse, f: (String, Seq[A]) => B)(implicit ex: ExecutionContext, mat: ActorMaterializer, reader: InfluxReader[A]): Future[QueryResult[B]] = {
    response.status.intValue() match {
      case code if isSuccessful(code) =>
        unmarshalBody(response)
          .map(getInfluxInfo[A])
          .map(seq => seq.map(e => f(e._1, e._2)))
          .map(seq => QueryResult[B](code, isSuccess = true, seq))
      case other => errorHandler(other, response).map(ex => QueryResult[B](other, isSuccess = false, ex = Some(ex)))
    }
  }
  ...
  def toQueryJsResult(response: HttpResponse)(implicit ex: ExecutionContext, mat: ActorMaterializer): Future[QueryResult[JsArray]] = {
    response.status.intValue() match {
      case code if isSuccessful(code) =>
        unmarshalBody(response).map(getInfluxValue).map(seq => QueryResult(code, isSuccess = true, seq))
      case other =>
        errorHandler(other, response).map(ex => QueryResult(other, isSuccess = false, ex = Some(ex)))
    }
  }
  ...
  def toBulkQueryJsResult(response: HttpResponse)(implicit ex: ExecutionContext, mat: ActorMaterializer): Future[QueryResult[Seq[JsArray]]] = {
    response.status.intValue() match {
      case code if isSuccessful(code) =>
        unmarshalBody(response).map(getBulkInfluxValue).map(seq => QueryResult[Seq[JsArray]](code, isSuccess = true, seq))
      case other =>
        errorHandler(other, response).map(ex => QueryResult(other, isSuccess = false, ex = Some(ex)))
    }
  }
