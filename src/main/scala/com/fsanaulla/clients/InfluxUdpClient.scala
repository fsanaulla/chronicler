...
import com.fsanaulla.model.{InfluxWriter, Point, UdpConnection}
import com.fsanaulla.utils.PointTransformer
...
private[fsanaulla] class InfluxUdpClient(host: String, port: Int = 8089) extends PointTransformer {
  ...
  private def send(dp: DatagramPacket) = socket.send(dp)
}
