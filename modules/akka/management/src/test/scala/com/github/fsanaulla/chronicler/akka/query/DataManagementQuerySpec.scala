...
class DataManagementQuerySpec extends FlatSpec with Matchers with DataManagementQuery[Uri] {
  ...
  it should "generate correct 'create database' query without auth" in new NonAuthEnv {
    createDatabaseQuery(testDb, Some("3d"), None, None, None).toString() shouldEqual queryTester(s"CREATE DATABASE $testDb WITH DURATION 3d")

    createDatabaseQuery(testDb, Some("3d"), Some(2), Some("1d"), Some("testName")).toString() shouldEqual queryTester(s"CREATE DATABASE $testDb WITH DURATION 3d REPLICATION 2 SHARD DURATION 1d NAME testName")
  }
  ...
  it should "generate auth correct 'drop measurement' query without auth" in new NonAuthEnv {
    dropMeasurementQuery(testDb, testMeasurement).toString() shouldEqual queryTester(testDb, s"DROP MEASUREMENT $testMeasurement")
  }
  ...
  it should "generate correct 'show tag-key' query without auth" in new NonAuthEnv {
    showTagKeysQuery(testDb, testMeasurement, None, None, None).toString() shouldEqual queryTester(s"SHOW TAG KEYS ON $testDb FROM $testMeasurement")
    showTagKeysQuery(testDb, testMeasurement, testWhereClause, testLimit, None).toString() shouldEqual queryTester(s"SHOW TAG KEYS ON $testDb FROM $testMeasurement WHERE ${testWhereClause.get} LIMIT ${testLimit.get}")
  }
  ...
  it should "generate correct 'show field-key' query without auth" in new NonAuthEnv {
    showFieldKeysQuery(testDb, testMeasurement).toString() shouldEqual queryTester(s"SHOW FIELD KEYS ON $testDb FROM $testMeasurement")
  }
}
